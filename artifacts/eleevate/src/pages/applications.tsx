...
function getInitialDemoApplications() {
  try {
    ...
    for (const app of [...stored, ...getDemoApplicationsFromShortlist()]) {
...
export default function ApplicationsPage() {
  ...
  const documentReadiness = applications.length === 0 ? 0 : Math.min(100, 42 + applications.length * 8 + offers.length * 7);
    ...
    <AppLayout>
      <div data-testid="applications-page">
        <PageHeader
          ...
          description={applications.length === 0 ? "Applications will appear here after you shortlist a university and choose a program." : `${applications.length} application${applications.length !== 1 ? "s" : ""} from research to offer, visa, and enrollment.`}
          actions={
            <>
              <Link href="/documents">
          ...
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="p-5">
              ...
              <h2 className="font-serif text-xl font-bold leading-tight text-foreground">
                {applications.length === 0 ? "Start by shortlisting universities. Each saved program can become a tracked application." : "Keep applications, deadlines, and document blockers in one operating view."}
              </h2>
            ...
            <aside className="border-t border-border bg-muted/35 p-4 lg:border-l lg:border-t-0">
              ...
              <div className="rounded-lg border border-[#F8B133]/35 bg-[#F8B133]/10 p-3">
                <div className="text-sm font-semibold leading-5 text-foreground">{applications.length === 0 ? "Explore universities and save the options you want to apply to." : "Review document requirements before submission."}</div>
                <Link href={applications.length === 0 ? "/universities" : "/documents"}>
                  <Button size="sm" variant="outline" className="mt-3 rounded-full bg-white">{applications.length === 0 ? "Explore universities" : "Open document packet"}</Button>
                </Link>
              </div>
            </aside>
          </div>
